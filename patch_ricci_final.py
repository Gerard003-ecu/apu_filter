...
"""
content = re.sub(r'    __slots__ = \(\n\s*"_critical",\n\s*"_warning",\n\s*"_hysteresis",\n\s*"_enable_topology_monitoring",\n\s*"_projector",\n\s*"_previous_status",\n\s*"_euler_history",\n\s*"_evaluation_count",\n\s*\)', slots_str.strip('\n'), content)
...
"""
content = re.sub(r'        # Estado mutable interno\n\s*self\._previous_status: Optional\[HealthStatus\] = None\n\s*self\._euler_history: List\[Optional\[int\]\] = \[\]\n\s*self\._evaluation_count: int = 0\n\n\s*self\._projector = self\._build_projector\(\)', init_str.strip('\n'), content)


# 3. Update _build_projector
proj_str = """
...
# 4. Update __call__ & Evolve logic
ricci_funcs = """
