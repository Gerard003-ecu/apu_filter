...
def process_files(presupuesto_path, apus_path, insumos_path):
    ...
    try:
        ...
        df_presupuesto = df_presupuesto.rename(
            columns={
                "ÍTEM": "CODIGO_APU",
        ...
        df_presupuesto["CANTIDAD_PRESUPUESTO"] = pd.to_numeric(
            df_presupuesto["CANTIDAD_PRESUPUESTO"], errors="coerce"
        )
        ...
        df_insumos = df_insumos.rename(
            columns={"CÓDIGO": "CODIGO_INSUMO", "VR.UNITARIO": "VR_UNITARIO_INSUMO"}
        )
        # Seleccionar solo las columnas necesarias
        df_insumos = df_insumos[["CODIGO_INSUMO", "VR_UNITARIO_INSUMO"]]
        df_insumos.dropna(subset=["CODIGO_INSUMO"], inplace=True)
        df_insumos["VR_UNITARIO_INSUMO"] = pd.to_numeric(
            df_insumos["VR_UNITARIO_INSUMO"], errors="coerce"
        )
        ...
        df_apus.dropna(
            subset=["CODIGO_APU", "CODIGO_INSUMO", "CANTIDAD_APU"], inplace=True
        )
