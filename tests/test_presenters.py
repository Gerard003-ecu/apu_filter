...
class TestAPUPresenterInitialization:
    ...
    def test_initialization_with_logger(self, mock_logger):
        ...
        mock_logger.info.assert_called_once_with(
            "APUPresenter inicializado correctamente"
        )
...
class TestDataSanitization:
    ...
    def test_sanitize_warns_about_negative_values(self, presenter, mock_logger):
        ...
        df = pd.DataFrame({
            'CANTIDAD_APU': [10, -20, 30],
            'VALOR_TOTAL_APU': [100, 200, -300]
        })
...
class TestSchemaValidation:
    ...
    def test_validate_schema_logs_success(self, presenter, valid_apu_details, mock_logger):
        ...
        mock_logger.debug.assert_called_with(
            "Validación de esquema exitosa para APU APU001"
        )
...
class TestAggregation:
    ...
    def test_group_by_category_consolidates_items(self, presenter, valid_apu_details):
        ...
        cement_items = [
            ...
            if item['DESCRIPCION'] == 'Cemento gris'
        ]
...
class TestBreakdownOrganization:
    ...
    def test_organize_breakdown_empty_list(self, presenter, mock_logger):
        ...
        mock_logger.warning.assert_called_with(
            "No hay items para organizar en desglose"
        )

    def test_organize_breakdown_custom_default_category(self, presenter_custom_config, mock_logger):
...
class TestProcessAPUDetailsIntegration:
    ...
    def test_process_apu_details_logs_processing(self, presenter, valid_apu_details, mock_logger):
    ...
    def test_process_apu_details_handles_whitespace(
        self, 
        presenter, 
        apu_details_whitespace_strings
    ):
        """Debe eliminar espacios en blanco de strings."""
        result = presenter.process_apu_details(
            apu_details_whitespace_strings, 
            "APU001"
        )
...
class TestEdgeCasesAndErrors:
    ...
    def test_process_missing_columns_raises_error(self, presenter):
        ...
        invalid_data = [
            {'CATEGORIA': 'MAT', 'DESCRIPCION_INSUMO': 'Cemento'}
        ]
    ...
    def test_process_with_inconsistent_prices(
        self, 
        presenter, 
        apu_details_inconsistent_prices,
        mock_logger
    ):
...
class TestParametrized:
    ...
    @pytest.mark.parametrize("invalid_apu_code", [
        "",
        "   ",
        None,
        123,
    ...
    @pytest.mark.parametrize("nan_value", [
        np.nan,
        np.inf,
        -np.inf,
        None
    ...
    @pytest.mark.parametrize("whitespace_string,expected", [
        ("  hello  ", "hello"),
        ("world  ", "world"),
        ("  test", "test"),
        ("no-space", "no-space"),
    ...
    def test_create_dataframe_exception_handling(self, presenter, mock_logger):
        ...
        with patch('pandas.DataFrame', side_effect=Exception("Mock error")):
            with pytest.raises(ValueError, match="Error creando DataFrame"):
                presenter._create_and_sanitize_dataframe(
                    [{'key': 'value'}], 
                    "APU001"
                )
