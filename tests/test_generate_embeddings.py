...
class TestDataValidator:
    def test_validate_dataframe_success(self, sample_dataframe, mock_config):
        ...
        validated_df = DataValidator.validate_dataframe(sample_dataframe, "description", "id", mock_config)
        assert len(validated_df) == 10
        assert "description" in validated_df.columns
        assert "id" in validated_df.columns

    def test_validate_dataframe_missing_column_raises_error(self, sample_dataframe, mock_config):
...
class TestEmbeddingGenerator:
    ...
    def test_load_model_failure_raises_error(self, mock_config):
        ...
        with patch('scripts.generate_embeddings.SentenceTransformer', side_effect=Exception("mock error")):
            generator = EmbeddingGenerator(mock_config)
...
def test_pipeline_run_integration(MockSentenceTransformer, mock_faiss, mock_config, sample_dataframe, tmp_path):
    """
    ...
    mock_index.search.side_effect = lambda query, k: (np.array([[0.99]]), np.array([[np.where(np.all(mock_embeddings == query, axis=1))[0][0]]]))
    mock_faiss.IndexFlatIP.return_value = mock_index
    ...
    with patch('scripts.generate_embeddings.psutil.virtual_memory') as mock_virtual_memory:
        ...
        mock_faiss.IndexFlatIP.assert_called_with(384)
        mock_index.add.assert_called_once()
        mock_faiss.write_index.assert_called_with(mock_index, str(mock_config.output_dir / "faiss.index"))
