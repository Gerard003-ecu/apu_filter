...
try:
    from ast_symplectic_parser import (
        AnalysisLimits,
        NodeCategory,
        DataFlowCoordinates,
        ComplexityProfile,
    ...
    print(f"Error importando ast_symplectic_parser: {e}")
    print("Asegúrate de que el archivo está en el mismo directorio o en PYTHONPATH")
    sys.exit(1)
...
"""
    ...
    def test_cyclomatic_lambda(self):
        ...
        self.assertEqual(profile.cyclomatic_complexity, 2)  # 1 base + 1 lambda
...
class TestJSONStructureValidator(TestBase):
    ...
    def test_compute_depth_nested(self):
        ...
        self.assertEqual(depth, 4)  # 1 + 1 + 1 + 1
    
    def test_compute_depth_empty(self):
        """Profundidad de estructura vacía."""
        self.assertEqual(JSONStructureValidator.compute_depth({}), 1)
        self.assertEqual(JSONStructureValidator.compute_depth([]), 1)
    
    def test_compute_depth_primitive(self):
        """Profundidad de primitivo."""
        self.assertEqual(JSONStructureValidator.compute_depth(42), 1)
        self.assertEqual(JSONStructureValidator.compute_depth("string"), 1)
...
"""
    ...
    def test_validate_json_contract_list(self):
        ...
        self.assertTrue(is_valid)
        self.assertIsNone(markdown)  # No se puede normalizar a tabla
...
class TestRiskProfile(TestBase):
    ...
    def test_risk_category(self):
        ...
        self.assertEqual(
            RiskProfile(risk_tolerance=0.1).risk_category,
            "HIGHLY_CONSERVATIVE"
        )
        self.assertEqual(
            RiskProfile(risk_tolerance=0.5).risk_category,
            "MODERATE"
        )
        self.assertEqual(
            RiskProfile(risk_tolerance=0.9).risk_category,
            "HIGHLY_AGGRESSIVE"
        )
...
class TestPurposeValidator(TestBase):
    ...
    def test_validate_non_canonical_problem(self):
        ...
        purposes = [
            BusinessPurpose("concept", "UNKNOWN_PROBLEM", strength=0.9)
        ]
...
class TestConfidenceFilter(TestBase):
    ...
    def test_validate_high_confidence(self):
        ...
        llm_output = LLMOutput(entropy=0.5, confidence=0.9, temperature=0.7, num_tokens=100)
    ...
    def test_validate_low_confidence(self):
        ...
        llm_output = LLMOutput(entropy=1.0, confidence=0.4, temperature=1.0, num_tokens=100)
    ...
    def test_validate_high_entropy(self):
        ...
        llm_output = LLMOutput(entropy=5.0, confidence=0.8, temperature=1.0, num_tokens=100)
...
class TestSemanticValidationEngine(TestBase):
    ...
    def test_validate_reject_low_confidence(self):
        ...
        llm_output = LLMOutput(entropy=5.0, confidence=0.3, temperature=1.5, num_tokens=100)
...
"""
        ...
        llm_output = LLMOutput(entropy=0.5, confidence=0.9, temperature=0.7, num_tokens=50)
...
"""
        ...
        engine = SemanticValidationEngine(
            risk_profile=RiskProfile(risk_tolerance=0.2)
        )
        ...
        llm_output = LLMOutput(entropy=0.8, confidence=0.85, temperature=0.8, num_tokens=150)
        ...
        self.assertIn(result.verdict, [Verdict.VIABLE, Verdict.CONDITIONAL, Verdict.WARNING])
