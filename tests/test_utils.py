...
class TestNormalizeTextSeries:
    ...
    def test_normalize_series_preserve_special(self):
        ...
        assert any("#" in str(val) or "@" in str(val) or "-" in str(val) for val in result)
...
class TestCleanApuCode:
    ...
    @pytest.mark.parametrize(
        "code, expected_clean, should_warn",
        [
            ("1", "1", False),
            ("1.2.3", "1.2.3", False),
            ("A.1", "A.1", False),
    ...
    def test_item_code_cleaning_and_warnings(
        self, code, expected_clean, should_warn, caplog
    ):
...
class TestCreateApuSignature:
    ...
    def test_signature_custom_fields(self):
        ...
        apu_data = {"CAMPO1": "Valor1", "CAMPO2": "Valor2"}
    ...
    def test_signature_numeric_values(self):
        ...
        apu_data = {"CODIGO_APU": 123, "DESCRIPCION_APU": "Test", "UNIDAD_APU": "M"}
        signature = utils.create_apu_signature(apu_data)
...
class TestDetectOutliers:
    ...
    def test_detect_outliers_with_bounds(self, sample_series_numeric):
        ...
        outliers, bounds = utils.detect_outliers(
            sample_series_numeric, method="iqr", return_bounds=True
        )
    ...
    def test_detect_outliers_modified_zscore(self, sample_series_numeric):
        ...
        outliers = utils.detect_outliers(
            sample_series_numeric, method="modified_zscore", threshold=3.5
        )
...
class TestFindAndRenameColumns:
    ...
    def test_rename_exact_match(self):
        ...
        column_map = {"CODIGO": ["codigo"], "DESCRIPCION": ["desc"]}
    ...
    def test_rename_partial_match(self):
        ...
        column_map = {"CODIGO": ["codigo"], "DESCRIPCION": ["descripcion"]}
    ...
    def test_rename_multiple_possibilities(self):
        ...
        column_map = {"CODIGO": ["codigo", "cod", "code"]}
...
class TestSanitizeForJson:
    ...
    def test_sanitize_nan_inf(self):
        ...
        data = {"nan": np.nan, "inf": np.inf, "neg_inf": -np.inf}
    ...
    def test_sanitize_nested_structures(self):
        ...
        data = {"level1": {"level2": {"level3": [np.int64(1), np.float32(2.5)]}}}
...
class TestIntegration:
    ...
    def test_full_workflow_text_processing(self):
        ...
        texts = pd.Series(["CÓDIGO APU-001", "código apu-002", "Código APU-003"])
    ...
    def test_full_workflow_dataframe_processing(self, sample_dataframe):
        ...
        # Renombrar columnas
        column_map = {"CODE": ["CODIGO", "codigo"], "DESC": ["DESCRIPCION", "descripcion"]}
        df = utils.find_and_rename_columns(sample_dataframe.copy(), column_map)

        # Validar valores numéricos
        valid_mask = utils.validate_series(
            df["VALOR_TOTAL_APU"], return_mask=True, min_value=0
        )
...
if __name__ == "__main__":
    ...
    pytest.main(
        [
            ...
            "-W",
            "ignore::DeprecationWarning",  # Ignorar warnings de deprecación
        ]
    )
