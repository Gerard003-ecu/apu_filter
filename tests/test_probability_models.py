...
def large_apu_data():
    ...
    return [
        {"VR_TOTAL": 10000.0 + i * 100, "CANTIDAD": 5.0 + i * 0.1}
        for i in range(1000)
    ]
...
class TestMonteCarloSimulatorDataPreparation:
    ...
    def test_prepare_data_with_threshold(self, valid_apu_data):
        ...
        config = MonteCarloConfig(
            min_cost_threshold=8000.0,
            min_quantity_threshold=4.0
        )
        simulator = MonteCarloSimulator(config=config)
...
class TestRunMonteCarloSimulationLegacy:
    ...
    def test_legacy_function_returns_correct_structure(self, valid_apu_data):
        ...
        assert set(result.keys()) == {
            "mean", "std_dev", "percentile_5", "percentile_95"
        }
...
class TestEdgeCases:
    ...
    def test_very_small_values(self, simulator):
        ...
        data = [
            {"VR_TOTAL": 0.01, "CANTIDAD": 0.01},
            {"VR_TOTAL": 0.02, "CANTIDAD": 0.02}
        ]
    ...
    def test_very_large_values(self, simulator):
        ...
        data = [
            {"VR_TOTAL": 1e12, "CANTIDAD": 1e6},
            {"VR_TOTAL": 1e11, "CANTIDAD": 1e5}
        ]
...
class TestPerformance:
    ...
    def test_extreme_load(self):
        ...
        data = [
            {"VR_TOTAL": 10000.0 + i, "CANTIDAD": 5.0 + i * 0.1}
...
class TestAdditionalCoverage:
    ...
    def test_create_no_data_result(self, simulator):
        ...
        result = simulator._create_no_data_result(
            total_items=10,
            discarded_items=10
        )
...
class TestFullIntegration:
    ...
    def test_legacy_to_modern_api_equivalence(self, valid_apu_data):
        ...
        assert abs(legacy_result["mean"] - modern_result.statistics["mean"]) < \
               modern_result.statistics["mean"] * 0.1  # 10% de tolerancia
