...
def run_monte_carlo_simulation(
    ...
    Args:
        ...
        volatility_factor: Factor de volatilidad relativo al costo base
                           (default: 0.10 = 10%).
        min_cost_threshold: Umbral mínimo para considerar un costo válido
                            (default: 0.0).
    ...
    if not isinstance(apu_details, list):
        ...
        return {"mean": None, "std_dev": None, "percentile_5": None, "percentile_95": None}
    ...
    if log_warnings and len(df) > 0:
        ...
        if discarded > 0:
            print(
                f"WARNING: Se descartaron {discarded} filas por valores inválidos "
    ...
    simulated_costs_matrix = np.random.normal(
        loc=base_costs, scale=scales, size=(num_simulations, len(base_costs))
    )
