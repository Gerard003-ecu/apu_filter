    ...
    def __post_init__(self):
        if self.required_columns is None:
            self.required_columns = {
                'CATEGORIA', 'DESCRIPCION_INSUMO', 'CANTIDAD_APU',
                'VALOR_TOTAL_APU', 'UNIDAD_APU', 'PRECIO_UNIT_APU',
                'CODIGO_APU', 'UNIDAD_INSUMO', 'RENDIMIENTO'
            }
        
        if self.numeric_columns is None:
            self.numeric_columns = {
                'CANTIDAD_APU', 'VALOR_TOTAL_APU', 
                'PRECIO_UNIT_APU', 'RENDIMIENTO'
            }
...
class APUPresenter:
    ...
    def process_apu_details(
        ...
        try:
            ...
            if not processed_items:
                self.logger.warning(
                    f"APU {apu_code}: No se generaron items procesados"
                )
            ...
            self.logger.error(
                f"Error de validación en APU {apu_code}: {str(e)}"
            )
            raise
        except Exception as e:
            self.logger.error(
                f"Error inesperado procesando APU {apu_code}: {str(e)}",
                exc_info=True
            )
            raise RuntimeError(
    ...
    def _validate_inputs(
        ...
        if not isinstance(apu_code, str) or not apu_code.strip():
            raise ValueError(
                f"apu_code debe ser un string no vacío, "
                f"recibido: '{apu_code}'"
            )

    def _create_and_sanitize_dataframe(
        self, 
        apu_details: List[Dict], 
        apu_code: str
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            raise ValueError(
                f"Error creando DataFrame para APU {apu_code}: {str(e)}"
            ) from e
        
        if df.empty:
            raise ValueError(
                f"DataFrame resultó vacío para APU {apu_code}"
            )
    ...
    def _validate_dataframe_schema(
        ...
        self.logger.debug(
            f"Validación de esquema exitosa para APU {apu_code}"
        )
    ...
    def _group_by_category(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        if len(categories) == 0:
            self.logger.warning(
                "No se encontraron categorías válidas en el DataFrame"
            )
            return processed
        ...
        for categoria in categories:
            ...
            try:
                ...
                self.logger.error(
                    f"Error procesando categoría '{categoria}': {str(e)}",
                    exc_info=True
                )
    ...
    def _aggregate_category_items(
        ...
        df_grouped = (
            df_categoria
            .groupby("DESCRIPCION_INSUMO", dropna=False)
            .agg(aggregations)
            .reset_index()
        )
    ...
    def _organize_breakdown(
        ...
        self.logger.debug(
            f"Desglose organizado en {len(desglose)} categorías"
        )
