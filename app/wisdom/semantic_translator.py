...
class Temperature:
    ...
    def __post_init__(self) -> None:
        ...
        # MEJORA: Eliminado código inalcanzable. El clamp de valores negativos
        # minúsculos se maneja en los classmethods from_celsius / from_kelvin.
        # Aquí solo validamos el invariante estricto T ≥ 0.
        if self.kelvin < 0.0:
            raise ValueError(
    ...
    def from_celsius(cls, celsius: float) -> Temperature:
        ...
        # Clamp valores negativos minúsculos por error numérico de punto flotante
    ...
    def is_absolute_zero(self) -> bool:
        ...
        MEJORA: Ahora usa comparación con tolerancia numérica en lugar de
        comparación exacta, coherente con el docstring y con la aritmética
...
class TopologyInvariantViolation(SemanticTranslatorError):
    r"""Violación de invariante topológico (χ ≠ β₀ − β₁ + β₂)."""
...
class StabilityThresholds:
    ...
    def __post_init__(self) -> None:
        ...
        if self.critical < 0:
            raise ValueError(
                f"critical threshold must be non-negative, got {self.critical}"
            )
        if not (self.critical < self.warning < self.solid):
            raise ValueError(
                f"Thresholds must satisfy strict ordering: "
                f"critical({self.critical}) < warning({self.warning}) "
                f"< solid({self.solid})"
            )
    ...
    def severity_score(self, stability: float) -> float:
        ...
        MEJORA: Función continua y monótona decreciente con saturación.
        Para Ψ ∈ [0, solid], score = 1 − Ψ/solid (lineal).
        Para Ψ ≥ solid, score = 0 (saturación).
        Para Ψ inválido, score = 1 (peor caso conservador).
        """
        if not math.isfinite(stability) or stability < 0:
            return 1.0
        if stability >= self.solid:
...
class TopologicalThresholds:
    ...
    # Umbrales espectrales (valor de Fiedler = λ₂ del Laplaciano)
    ...
    def __post_init__(self) -> None:
        if self.connected_components_optimal < 1:
            raise ValueError(
                f"optimal connected components must be ≥ 1, "
                f"got {self.connected_components_optimal}"
            )
        if self.cycles_optimal < 0:
            raise ValueError(
                f"optimal cycles must be non-negative, "
                f"got {self.cycles_optimal}"
            )
        if not (self.cycles_optimal <= self.cycles_warning <= self.cycles_critical):
            raise ValueError(
                f"Cycle thresholds must be ordered: "
                f"{self.cycles_optimal} ≤ {self.cycles_warning} "
                f"≤ {self.cycles_critical}"
            )
        if self.max_fragmentation < self.connected_components_optimal:
            raise ValueError(
                f"max_fragmentation ({self.max_fragmentation}) must be ≥ "
                f"connected_components_optimal "
                f"({self.connected_components_optimal})"
            )
        ...
        if self.fiedler_connected_threshold >= self.fiedler_robust_threshold:
            raise ValueError(
                f"fiedler_connected_threshold "
                f"({self.fiedler_connected_threshold}) "
                f"must be < fiedler_robust_threshold "
                f"({self.fiedler_robust_threshold})"
            )
    ...
    def classify_spectral_connectivity(self, fiedler_value: float) -> str:
        ...
        λ₂ grande ⟹ alta robustez de conectividad.
        """
    ...
    def validate_euler_characteristic(
        ...
        NOTA MATEMÁTICA: Para un grafo conexo (1-complejo simplicial),
        χ = V − E = β₀ − β₁ (con β₂ = 0). Esto es equivalente a la
        fórmula de Euler generalizada via el Teorema de Euler-Poincaré.
        """
    ...
    def __post_init__(self) -> None:
        ...
        for i, t in enumerate(temps):
            if not math.isfinite(t):
                raise ValueError(
                    f"Temperature threshold at index {i} must be finite"
                )
        ...
        if not (0 <= self.entropy_low < self.entropy_high
                < self.entropy_death <= 1):
            raise ValueError(
                f"Entropy thresholds must satisfy: "
                f"0 ≤ {self.entropy_low} < {self.entropy_high} "
                f"< {self.entropy_death} ≤ 1"
            )
        ...
        if self.heat_capacity_minimum <= 0:
            raise ValueError(
                f"heat_capacity_minimum must be positive, "
                f"got {self.heat_capacity_minimum}"
            )
...
class FinancialThresholds:
    ...
    def __post_init__(self) -> None:
        ...
        if not (0 < self.profitability_marginal < self.profitability_good
                < self.profitability_excellent):
            raise ValueError(
                f"Profitability thresholds must be strictly increasing and "
                f"positive: {self.profitability_marginal}, "
                f"{self.profitability_good}, {self.profitability_excellent}"
            )
        if not (0 < self.contingency_minimal < self.contingency_standard
                < self.contingency_high):
            raise ValueError(
                f"Contingency thresholds must be strictly increasing and "
                f"positive: {self.contingency_minimal}, "
                f"{self.contingency_standard}, {self.contingency_high}"
            )
...
class TranslatorConfig:
    ...
    stability: StabilityThresholds = field(
        default_factory=StabilityThresholds
    )
    topology: TopologicalThresholds = field(
        default_factory=TopologicalThresholds
    )
    thermal: ThermalThresholds = field(default_factory=ThermalThresholds)
    financial: FinancialThresholds = field(
        default_factory=FinancialThresholds
    )
...
class VerdictLevel(IntEnum):
    ...
    def verify_lattice_laws(cls) -> Dict[str, bool]:
        r"""
        Verifica exhaustivamente las leyes del retículo para todos los
        elementos.

        Complejidad: O(n³) donde n = |VerdictLevel| = 5.

        MEJORA: Se añade verificación de complemento dual y se documenta
        la completitud del conjunto de axiomas verificados.
        ...
        # 5. Identidad (⊥ para ⊔, ⊤ para ⊓)
        results["identity_join_bottom"] = all(
            a | cls.bottom() == a for a in elems
        )
        results["identity_meet_top"] = all(
            a & cls.top() == a for a in elems
        )
        ...
        # MEJORA 9. Acotamiento: ∀a, ⊥ ≤ a ≤ ⊤
        results["bounded"] = all(
            cls.bottom() <= a <= cls.top() for a in elems
        )

        # MEJORA 10. Totalidad del orden: ∀a,b, a ≤ b ∨ b ≤ a
        results["total_order"] = all(
            a <= b or b <= a for a in elems for b in elems
        )
...
class SeverityLattice(IntEnum):
    ...
    # MEJORA: Añadido infimum para completitud algebraica del retículo
    @classmethod
    def infimum(cls, *levels: SeverityLattice) -> SeverityLattice:
        """⊓ (meet) del retículo de severidad."""
        if not levels:
            return cls.RECHAZAR
        return cls(min(l.value for l in levels))
...
class SeverityToVerdictHomomorphism:
    ...
    MEJORA: Se añade verificación de preservación de ⊥ y ⊤ (condición
    necesaria para homomorfismos de retículos acotados).
    """
    ...
    def verify_homomorphism(cls) -> bool:
        r"""
        Verifica formalmente que φ preserva join, meet y elementos
        distinguidos.

        MEJORA: Se añade verificación de preservación de bottom y top:
            φ(⊥_S) = ⊥_V  y  φ(⊤_S) = ⊤_V
        """
        all_sev = list(SeverityLattice)

        # Preservación de ⊥ y ⊤
        if cls.apply(SeverityLattice.VIABLE) != VerdictLevel.bottom():
            return False
        if cls.apply(SeverityLattice.RECHAZAR) != VerdictLevel.top():
            return False
...
class SemanticDiffeomorphismMapper:
    ...
    def map_betti_1_cycles(cycle_nodes: List[str]) -> str:
        ...
        return (
            f"VETO ESTRUCTURAL (Socavón Lógico): Se ha detectado una "
            f"dependencia circular infinita en la ruta crítica. El sistema "
            f"de compras se paralizará porque la adquisición de estos "
    ...
    @staticmethod
    def map_pyramid_instability(
        psi_value: float, critical_supplier: str,
    ) -> str:
        r"""
        ...
        """
        return (
            f"ALERTA DE QUIEBRA (Pirámide Invertida): Su presupuesto "
            f"descansa sobre una base logística peligrosamente estrecha "
...
class GraphRAGCausalNarrator:
    ...
    def __init__(
        ...
        if not isinstance(knowledge_graph, nx.DiGraph):
            raise GraphStructureError(
                f"Expected nx.DiGraph, got "
                f"{type(knowledge_graph).__name__}"
            )
    ...
    def narrate_topological_collapse(
        ...
        if len(acta_sections) == 1:
            acta_sections.append(
                "ESTADO NOMINAL: La topología del presupuesto es "
                "laminar y coherente."
            )
    ...
    def extract_causality(self) -> List[List[str]]:
        r"""
        Extrae causalidades identificando ciclos, usando la base de ciclos
        fundamental para deduplicación rigurosa.

        MEJORA: Reemplazada heurística de Jaccard por base de ciclos
        fundamental (cycle basis) del grafo no dirigido subyacente.
        La dimensión del espacio de ciclos es dim(Z₁) = |E| − |V| + β₀,
        y la base fundamental genera exactamente β₁ ciclos independientes.
        """
        cycles = self._enumerate_cycles_bounded()
        if not cycles:
            return []

        # Usar la base de ciclos fundamental como representantes canónicos
        # de las clases de homología H₁(G; ℤ)
        ...
        if not basis:
            # Fallback: retornar ciclos dirigidos deduplicados por soporte
            return self._deduplicate_cycles_by_support(cycles)

        # Para cada ciclo de la base, encontrar el ciclo dirigido que
        # mejor lo representa (máxima intersección normalizada)
        ...
        for c in cycles:
            ...
            for idx, bc_set in enumerate(basis_sets):
                ...
                intersection = len(c_set & bc_set)
                union = len(c_set | bc_set)
            ...
            if best_idx >= 0 and best_overlap >= 0.5:
                if best_idx not in used_basis_indices:
                    unique_cycles.append(c)
                    used_basis_indices.add(best_idx)
            elif best_idx < 0:
                # Ciclo dirigido sin correspondencia en la base no dirigida
                unique_cycles.append(c)

        return unique_cycles if unique_cycles else cycles[:1]

    def _deduplicate_cycles_by_support(
        self, cycles: List[List[str]],
    ) -> List[List[str]]:
        """
        Fallback: deduplica ciclos por soporte (conjunto de nodos).

        MEJORA: Protección explícita contra división por cero y uso de
        frozenset para comparación eficiente O(1).
        """
        unique_cycles: List[List[str]] = []
        ...
        for c, c_set in cycle_sets:
            ...
            for seen in seen_supports:
                ...
                if union_size == 0:
                    continue
                intersection_size = len(c_set & seen)
                if intersection_size / union_size >= 0.5:
                    is_duplicate = True
                    break
            if not is_duplicate:
                unique_cycles.append(c)
                seen_supports.append(c_set)
        ...
        try:
            for i, cycle in enumerate(nx.simple_cycles(self.kg)):
                if i >= self._max_cycles:
                    logger.warning(
                        f"Cycle enumeration truncated at "
                        f"{self._max_cycles} cycles"
                    )
                    break
    ...
    def _find_critical_node(self) -> str:
        ...
        NOTA MATEMÁTICA: La centralidad de autovector corresponde al
        autovector de Perron-Frobenius de la matriz de adyacencia A.
        Solo está bien definida para grafos fuertemente conexos (teorema
        de Perron-Frobenius generalizado).
        """
        if self.kg.number_of_nodes() == 0:
        ...
        except (nx.NetworkXError, ArithmeticError, np.linalg.LinAlgError) as e:
...
class LatticeVerdictCollapse:
    r"""
    Motor determinista que subordina las decisiones del LLM al rigor
    matemático.
    ...
    def compute_supremum(
        ...
        MEJORA: Simplificado usando directamente SeverityLattice.supremum()
        ya que la optimización del elemento absorbente es redundante con
        la operación max subyacente (max con ⊤ siempre es ⊤).
        """
    ...
    def enforce_semantic_diffeomorphism(
        ...
        Colapsa la función de decisión y emite el Acta de Deliberación
        inmutable.
        ...
        if betti_1 > 0 or psi < 1.0:
            topo_verdict = SeverityLattice.RECHAZAR

        # Absorbing element check (⊤)
        if topo_verdict == SeverityLattice.RECHAZAR:
            fin_verdict = SeverityLattice.VIABLE  # Absorbed by ⊤
        ...
        if (
            ...
            synthesis += (
                "JUSTIFICACIÓN: Se ha aplicado un cortocircuito absorbente "
                "(Fast-Fail). Debido a que la topología devuelve un estado "
                "de RECHAZAR (⊤), el sistema omite la evaluación financiera, "
                "ya que a ⊔ ⊤ = ⊤. El rigor topológico/físico domina "
                "incondicionalmente."
            )
...
class FinancialVerdict(Enum):
    ...
    # MEJORA: _ALIASES como ClassVar para evitar reconstrucción en cada llamada
...
class ValidatedTopology:
    ...
    def __post_init__(self) -> None:
        ...
        for name, val in [
            ...
            if val < 0:
                raise ValueError(
                    f"Betti number {name} must be non-negative, got {val}"
                )
        ...
        if (math.isfinite(self.fiedler_value)
                and self.fiedler_value < -EPSILON):
            raise ValueError(
                f"Fiedler value (λ₂ of Laplacian) must be ≥ 0, "
                f"got {self.fiedler_value}"
            )

        # Validar estabilidad piramidal
        if (math.isfinite(self.pyramid_stability)
                and self.pyramid_stability < 0):
            raise ValueError(
                f"Pyramid stability Ψ must be non-negative, "
                f"got {self.pyramid_stability}"
            )

        # Validar entropía estructural
        if (math.isfinite(self.structural_entropy)
                and self.structural_entropy < 0):
            raise ValueError(
                f"Structural entropy must be non-negative, "
                f"got {self.structural_entropy}"
            )
    ...
    def from_metrics(
        ...
        Si ``strict=False``, corrige la característica de Euler
        automáticamente y emite un warning estructurado.
        ...
        if fiedler < 0:
            logger.warning(
                ...
                f"(probable numerical error in Laplacian "
...
class NarrativeCache:
    ...
    def _make_key(
        ...
        r"""
        Genera clave determinista usando SHA-256 sobre representación
        canónica.

        MEJORA: Incluye tensor topológico-térmico (β₀, β₁, λ₂, Ψ, T)
        para garantizar isomorfismo semántico. Se usa json canónico con
        sort_keys y separadores mínimos para determinismo inter-proceso.
        """
    ...
    def stats(self) -> Dict[str, int]:
        ...
        with self._lock:
            ...
            return {
                ...
                "misses": self._misses,
                "hit_rate_pct": (
                    round(100 * self._hits / total, 1) if total > 0
                    else 0.0
                ),
            }
...
class SemanticTranslator:
    r"""
    Traductor semántico que convierte métricas técnicas en narrativa de
    ingeniería.
    ...
    def __init__(
        ...
        self._last_fiedler: Optional[float] = None
        self._last_verdict: Optional[VerdictLevel] = None
        self._fiedler_hysteresis_band: float = 1e-12
    ...
    def _mahalanobis_retrieve(
        ...
        r"""
        Recuperación semántica anisotrópica acoplada al Tensor Métrico
        de Riesgo.

        Calcula la distancia de Mahalanobis:
            d(x, y) = sqrt((x-y)^T G (x-y))

        MEJORA: Se añade validación de:
        1. Dimensionalidad compatible entre query, candidatos y tensor
        2. Simetría del tensor G (propiedad necesaria de una forma bilineal)
        3. Semi-definitud positiva de G (d² ≥ 0 para toda diferencia)

        Args:
            query_vector: Vector de intención (ℝⁿ).
            domain_tensor: Tensor G_{μν} simétrico positivo semi-definido
                          (n×n).
            candidates: Lista de tuplas (texto, vector ∈ ℝⁿ).

        Returns:
            El texto del candidato que minimice la energía cuadrática.
        """
        if not candidates:
            return ""

        n = query_vector.shape[0]

        # Validación de dimensionalidad
        if domain_tensor.shape != (n, n):
            logger.warning(
                f"Metric tensor shape {domain_tensor.shape} incompatible "
                f"with query dimension {n}. Falling back to Euclidean."
            )
            domain_tensor = np.eye(n)

        # Validación de simetría (||G - G^T||_F < ε)
        asymmetry = np.linalg.norm(
            domain_tensor - domain_tensor.T, ord='fro'
        )
        if asymmetry > EPSILON:
            logger.warning(
                f"Metric tensor asymmetry ||G - G^T||_F = "
                f"{asymmetry:.2e} > ε. Symmetrizing."
            )
            domain_tensor = (domain_tensor + domain_tensor.T) / 2.0

        best_candidate = candidates[0][0]
        min_distance = float('inf')

        for text, candidate_vector in candidates:
            if candidate_vector.shape[0] != n:
                logger.warning(
                    f"Candidate vector dimension {candidate_vector.shape[0]} "
                    f"≠ query dimension {n}. Skipping."
                )
                continue
            diff = query_vector - candidate_vector
            dist_sq = float(diff.T @ domain_tensor @ diff)

            # MEJORA: Validar semi-definitud positiva implícitamente
            if dist_sq < -EPSILON:
                logger.warning(
                    f"Negative Mahalanobis distance² = {dist_sq:.6e}. "
                    f"Metric tensor may not be positive semi-definite."
                )
                dist_sq = abs(dist_sq)
    ...
    def _fetch_narrative(
        ...
        if ("query_vector" in params and "domain_tensor" in params
                and "candidates" in params):
        ...
        narrative = response.get(
            "narrative", f"[{domain}.{classification}]"
        )
    ...
    def _normalize_temperature(
        ...
        if not math.isfinite(value):
            logger.warning(
                f"Non-finite temperature value {value}, "
                f"defaulting to 298.15 K"
            )
    ...
    def explain_stress_point(
        ...
        """Explica por qué un nodo es punto de estrés crítico."""
    ...
    def translate_topology(
        ...
        # Recuperación Mahalanobis opcional
        mahalanobis_context: Dict[str, Any] = {}
        try:
            from app.core.immune_system.metric_tensors import G_TOPOLOGY
            mahalanobis_context = {
                "domain_tensor": G_TOPOLOGY,
                "query_vector": np.array(
                    [topo.beta_0, topo.beta_1], dtype=np.float64
                ),
                "candidates": [
        ...
        eff_stability = (
            stability if stability != 0.0 else topo.pyramid_stability
        )

        # Histéresis semántica en Fiedler (Schmitt Trigger Topológico)
        current_fiedler = topo.fiedler_value
        if (self._last_fiedler is not None
                and self._last_verdict is not None):
            if (abs(current_fiedler - self._last_fiedler)
                    <= self._fiedler_hysteresis_band):
        ...
        if not math.isfinite(eff_stability) or eff_stability < 0:
            raise MetricsValidationError(
                f"Stability Ψ must be a non-negative finite number, "
                f"got {eff_stability}"
            )
        ...
        if mahalanobis_context:
            ...
            cycle_verdict = (
                VerdictLevel.RECHAZAR if topo.beta_1 > 1
                else (
                    VerdictLevel.PRECAUCION if topo.beta_1 > 0
                    else VerdictLevel.VIABLE
                )
            )
        else:
            cycle_narrative, cycle_verdict = self._translate_cycles(
                topo.beta_1
            )
        ...
        if synergy_detected:
            ...
            if bridge_nodes:
                ...
                explanation = self.explain_stress_point(
                    node_id, "múltiples"
                )
                if explanation:
        ...
        conn_narrative, conn_verdict = self._translate_connectivity(
            topo.beta_0
        )
        ...
        stab_narrative, stab_verdict = self._translate_stability(
            eff_stability
        )
    ...
    def _translate_cycles(
        ...
        return narrative, verdict_map.get(
            classification, VerdictLevel.REVISAR
        )

    def _translate_connectivity(
        self, beta_0: int,
    ) -> Tuple[str, VerdictLevel]:
    ...
    def _translate_stability(
        self, stability: float,
    ) -> Tuple[str, VerdictLevel]:
        """Traduce Ψ (índice de estabilidad piramidal)."""
        classification = self.config.stability.classify(stability)
        if classification == "invalid":
            return (
                "⚠ **Valor de estabilidad inválido**",
    ...
    def _translate_spectral(
        ...
        parts: List[str] = []
        cohesion_class = (
            self.config.topology.classify_spectral_connectivity(fiedler)
        )
        ...
        parts.append(
            self._fetch_narrative(
                "SPECTRAL_COHESION", cohesion_type,
    ...
    def translate_thermodynamics(
        ...
        Difusión Térmica Laplaciana:
            T_eff = T_base · exp(−λ₂ · t)
        donde λ₂ es el valor de Fiedler y t = 1 (paso temporal unitario).

        Si λ₂ < ε (grafo desconectado), el calor no se difunde:
        embotellamiento térmico.
        """
        if isinstance(metrics, dict):
        ...
        # Difusión Térmica Laplaciana
        t = 1.0
        effective_temp_kelvin = base_temp.kelvin * math.exp(
            -fiedler_value * t
        )

        # Embotellamiento térmico por baja conectividad
        is_thermal_bottleneck = (
            fiedler_value
            < self.config.topology.fiedler_connected_threshold
        )
        ...
        entropy = (
            max(0.0, min(1.0, thermo.entropy))
            if math.isfinite(thermo.entropy) else 0.0
        )
        exergy = (
            max(0.0, min(1.0, thermo.exergetic_efficiency))
            if math.isfinite(thermo.exergetic_efficiency) else 0.0
        )
        ...
        verdicts.append(
            exergy_verdict_map.get(exergy_class, VerdictLevel.CONDICIONAL)
        )
        ...
        if is_thermal_bottleneck:
            parts.append(
                f"🔥 **Embotellamiento Térmico Inminente**: La "
                f"conectividad algebraica es críticamente baja "
                f"(λ₂ = {fiedler_value:.2e}). El calor no puede "
                f"disiparse por la red, creando singularidades "
                f"térmicas locales."
            )
            verdicts.append(VerdictLevel.RECHAZAR)
        else:
            ...
            verdicts.append(
                temp_verdict_map.get(temp_class, VerdictLevel.REVISAR)
            )
            if temp_class in ("hot", "critical"):
                parts.append(
                    "🍳 **Receta**: Se recomienda enfriar mediante "
                    "contratos de futuros o stock preventivo."
                )

        # 4. Inercia (Capacidad Calorífica)
        heat_cap = (
            thermo.heat_capacity
            if math.isfinite(thermo.heat_capacity) else 0.0
        )
    ...
    def _validate_financial_metrics(
        ...
        if not isinstance(metrics, dict):
            raise MetricsValidationError(
                f"Expected dict for financial metrics, "
    ...
    def get_market_context(self) -> str:
        ...
        context = response.get(
            "narrative", "Datos de mercado no disponibles."
        )
        return f"🌍 **Suelo de Mercado**: {context}"
    ...
    def _analyze_physics_stratum(
        ...
        if physics is not None:
            ...
            if not math.isfinite(gyro):
                ...
                narrative_parts.append(
                    self._fetch_narrative(
                        "GYROSCOPIC_STABILITY", "precession"
                    )
                )
                verdicts.append(VerdictLevel.PRECAUCION)
            else:
        ...
        if control is not None:
            if not control.is_stable:
                ...
                issues.append(
                    f"Estabilidad Marginal "
                    f"(PM={control.phase_margin_deg:.1f}°)"
                )
                narrative_parts.append(
        ...
        # 3. Análisis térmico con difusión Laplaciana
        base_temp = self._normalize_temperature(
            thermal.system_temperature
        )
        t = 1.0
        effective_temp_kelvin = base_temp.kelvin * math.exp(
            -fiedler_value * t
        ...
        if is_thermal_bottleneck:
            issues.append(
                f"Embotellamiento Térmico Inminente "
        ...
        if physics is not None:
            pressure = (
                physics.pressure
                if math.isfinite(physics.pressure) else 0.0
            )
            saturation = (
                physics.saturation
                if math.isfinite(physics.saturation) else 0.0
            )
            if pressure > 0.7:
                issues.append(
        ...
        verdict = (
            VerdictLevel.supremum(*verdicts) if verdicts
            else VerdictLevel.VIABLE
        )

        base_narratives: Dict[VerdictLevel, str] = {
            VerdictLevel.VIABLE: (
                "Base física estable. Flujo de datos sin turbulencia."
            ),
            VerdictLevel.RECHAZAR: (
                "Inestabilidad física crítica. Datos no confiables."
            ),
        }
        ...
        full_narrative = (
            f"{base_narrative} {' '.join(narrative_parts)}".strip()
        )
        ...
        metrics_summary: Dict[str, Any] = {
            ...
            "entropy": entropy_val,
            "exergy": (
                thermal.exergy if math.isfinite(thermal.exergy) else 0.0
            ),
            "stability": stability,
        }
        if physics is not None:
            metrics_summary["gyroscopic_stability"] = (
                physics.gyroscopic_stability
            )
            metrics_summary["pressure"] = physics.pressure
        if control is not None:
            metrics_summary["phase_margin_deg"] = (
                control.phase_margin_deg
            )
    ...
    def _analyze_tactics_stratum(
        ...
        if cycle_class != "clean":
            issues.append(
                f"Socavón lógico detectado (β₁={topo.beta_1})"
            )
            verdict_map: Dict[str, VerdictLevel] = {
                "critical": VerdictLevel.RECHAZAR,
                "moderate": VerdictLevel.PRECAUCION,
            ...
            verdicts.append(
                verdict_map.get(cycle_class, VerdictLevel.CONDICIONAL)
            )
        else:
            verdicts.append(VerdictLevel.VIABLE)
        ...
        if stab_class == "critical":
            ...
            issues.append(
                f"Estabilidad marginal (Ψ={stability:.2f})"
            )
            verdicts.append(VerdictLevel.PRECAUCION)
        ...
        if verdict == VerdictLevel.VIABLE:
            ...
            if any("Silos" in i or "desconectado" in i for i in issues):
                narrative = (
                    f"Silos organizacionales detectados. La estructura "
                    f"está gravemente fragmentada. "
                    f"Detalles: {issue_summary}"
                )
            elif any("Socavón" in i for i in issues):
                narrative = (
                    f"Socavón lógico detectado. La estructura contiene "
                    f"ciclos anómalos. Detalles: {issue_summary}"
                )
            else:
                narrative = (
            ...
            narrative = (
                f"Estructura con defectos menores a corregir: "
                f"{'; '.join(issues)}"
            )
    ...
    def _analyze_strategy_stratum(
        ...
        try:
            ...
            if verdict.is_positive:
                ...
                narrative = (
                    "Proyecto no es financieramente viable en "
            ...
            logger.error(
                f"Error analyzing strategy stratum: {e}",
                exc_info=True,
            )
            return StratumAnalysisResult(
                stratum=Stratum.STRATEGY,
    ...
    def _analyze_wisdom_stratum(
        ...
        """Genera el análisis del estrato WISDOM."""
    ...
    def _generate_report_header(self) -> str:
        ...
        return (
            ...
            f"Estabilidad Crítica: "
            f"Ψ < {self.config.stability.critical}*\n"
        )
    ...
    def _generate_integrated_diagnosis(
        ...
        temp_class = self.config.thermal.classify_temperature(
            temperature_celsius
        )
        if temp_class in ("hot", "critical"):
            parts.append(
                f"Sin embargo, el entorno es hostil. Detectamos una "
                f"**Fiebre Inflacionaria** de "
                f"{temperature_celsius:.1f}°C entrando por los insumos "
                f"(volatilidad de precios)."
            )

        dissipation_narratives: Dict[str, str] = {
            "robust": (
                "✅ Gracias a la base ancha, la estructura actúa como "
                "un **disipador de calor** eficiente. El riesgo de "
                "sobrecostos se diluye en la red de proveedores "
                "resiliente."
            ),
            "critical": (
        ...
        default_dissipation = (
            "⚠ La estructura tiene capacidad moderada de disipación, "
        ...
        if temp_class not in ("hot", "critical"):
            parts.append(
                f"El entorno es térmicamente estable "
    ...
    def _generate_final_advice(
        ...
        if synergy_detected:
            ...
            if (intersecting_cycles
                    and isinstance(intersecting_cycles[0], list)):
                cycle_explanation = self.explain_cycle_path(
                    intersecting_cycles[0]
                )
        ...
        try:
            fin_verdict = self._validate_financial_metrics(
                financial_metrics
            )["recommendation"]
        ...
        if is_inverted:
            ...
            return self._fetch_narrative(
                "FINAL_VERDICTS", "inverted_pyramid_reject"
            )

        if topo.has_cycles:
            return self._fetch_narrative(
                "FINAL_VERDICTS", "has_holes",
                {"beta_1": topo.beta_1},
            )
    ...
    def _generate_executive_summary(
        ...
        if errors:
            parts.append(
                f"\n⚠ Se detectaron {len(errors)} errores "
                f"durante el análisis."
            )
        return "\n".join(parts)
    ...
    def _generate_recommendations(
        ...
        for stratum, analysis in strata.items():
            if (analysis.verdict.requires_attention
                    or analysis.verdict.is_negative):
                if stratum == Stratum.PHYSICS:
                    recommendations.append(
                        "🔧 Revisar fuentes de datos y estabilizar "
                    ...
                    if isinstance(beta_1, int) and beta_1 > 0:
                        recommendations.append(
                            "🔄 Sanear topología eliminando ciclos "
                            "de dependencia."
                        )
        ...
        if not recommendations:
            recommendations.append(
                "🔍 Realizar análisis adicional para determinar "
                "siguiente paso."
            )
    ...
    def compose_strategic_narrative(
        self,
        topological_metrics: Union[
            TopologicalMetrics, Dict[str, Any], ValidatedTopology
        ],
        financial_metrics: Dict[str, Any],
        stability: float = 0.0,
        synergy_risk: Optional[Dict[str, Any]] = None,
        spectral: Optional[Dict[str, Any]] = None,
        thermal_metrics: Optional[
            Union[ThermodynamicMetrics, Dict[str, Any]]
        ] = None,
        physics_metrics: Optional[
            Union[PhysicsMetrics, Dict[str, Any]]
        ] = None,
        control_metrics: Optional[
            Union[ControlMetrics, Dict[str, Any]]
        ] = None,
        ...
        **kwargs: Any,
        ...
        r"""
        Compone el reporte ejecutivo completo integrando todos los
        estratos DIKW.
        ...
        if isinstance(tm, ThermodynamicMetrics):
            ...
            thermal = (
                ThermodynamicMetrics(**safe_fields) if safe_fields
                else ThermodynamicMetrics()
            )
        else:
            thermal = ThermodynamicMetrics()
        ...
        # ====== Topología ======
        section_narratives.append(
            "### 1. Auditoría de Integridad Estructural\n"
        )
        try:
        ...
        # ====== Finanzas ======
        section_narratives.append(
            "### 2. Análisis de Cargas Financieras\n"
        )
        try:
            fin_narrative, fin_verdict, _ = self.translate_financial(
                financial_metrics
            )
        ...
        graph_for_narrator: nx.DiGraph = kwargs.get(
            "graph", nx.DiGraph()
        )
        ...
        omega_narrative = (
            LatticeVerdictCollapse.enforce_semantic_diffeomorphism(
        ...
        # Verificar coherencia del homomorfismo
        severity_verdict = (
            SeverityLattice.RECHAZAR
            if (topo.beta_1 > 0 or eff_stability < 1.0)
            else SeverityLattice.VIABLE
        )
        mapped_verdict = SeverityToVerdictHomomorphism.apply(
            severity_verdict
        )
        ...
        # Registrar estrato ALPHA (si existe)
        if hasattr(Stratum, "ALPHA"):
            alpha_result = StratumAnalysisResult(
                ...
                metrics_summary={
                    "business_valid": final_verdict.is_positive,
                },
                issues=[],
                confidence=omega_confidence,
            )
        ...
        confidences = [
            a.confidence for a in strata_analysis.values()
        ]
        global_confidence = (
        ...
        if len(raw_narrative) > self.config.max_narrative_length:
            raw_narrative = (
                raw_narrative[:self.config.max_narrative_length - 3]
                + "..."
            )
...
def verify_severity_homomorphism() -> bool:
    ...
    if not result:
        logger.error(
            "SeverityLattice → VerdictLevel homomorphism is invalid"
        )
    return result