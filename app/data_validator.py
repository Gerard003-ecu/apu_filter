...
def _agregar_alerta(
    ...
    alerta_completa = {"tipo": tipo_alerta.value, "mensaje": mensaje}
...
def _validar_campos_requeridos(
    item: Dict[str, Any], campos_requeridos: List[str], nombre_item: str = "desconocido"
) -> List[str]:
    """
    ...
    for idx, insumo in enumerate(result):
        ...
        if puede_fuzzy_match:
            ...
            razon = (
                ...
                if HAS_FUZZY
                else "fuzzy matching no está instalado"
            )
            alerta_msg = (
                f"Descripción faltante (valor original: {descripcion_original}). "
...
def validate_and_clean_data(data_store: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "presupuesto" in result:
        ...
        try:
            ...
            logger.error(f"Error crítico al validar presupuesto: {str(e)}", exc_info=True)
    ...
    tiene_errores = any(
        "error" in m for m in metricas_totales.values() if m and isinstance(m, dict)
    )
    ...
    resumen = {
        ...
        "validaciones_realizadas": [k for k, v in metricas_totales.items() if v is not None],
        "mensaje": (
