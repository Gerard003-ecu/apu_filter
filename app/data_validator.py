...
def _validate_extreme_costs(presupuesto_data: list) -> list:
    ...
    for item in presupuesto_data:
        if item.get("VALOR_CONSTRUCCION_UN", 0) > COSTO_MAXIMO_RAZONABLE:
            costo_unitario = item.get("VALOR_CONSTRUCCION_UN")
            alerta_msg = (
                f"Costo unitario ({costo_unitario:,.2f}) excede el umbral de "
                f"{COSTO_MAXIMO_RAZONABLE:,.2f}."
            )
            item["alerta"] = alerta_msg
            logger.warning(
                f"Alerta de costo excesivo para el item {item.get('ITEM')}: {alerta_msg}"
            )
    ...
    for insumo in apus_detail_data:
        if insumo.get("CANTIDAD") == 0 and insumo.get("VALOR_TOTAL", 0) > 0:
            ...
            if precio_unitario > 0:
                ...
                alerta_msg = (
                    f"Cantidad recalculada a {nueva_cantidad:.4f} (era 0 con costo)."
                )
                insumo["alerta"] = alerta_msg
                logger.info(
                    f"Insumo {insumo.get('DESCRIPCION_INSUMO')} con cantidad 0 y costo. "
                    f"Recalculada a {nueva_cantidad}."
                )
            else:
                alerta_msg = (
                    "Cantidad es 0 con costo, pero el precio unitario es 0. "
                    "No se puede recalcular."
                )
                insumo["alerta"] = alerta_msg
                logger.warning(
                    "No se pudo recalcular la cantidad para el insumo "
                    f"{insumo.get('DESCRIPCION_INSUMO')}."
                )
...
def _validate_missing_descriptions(
    ...
    if raw_insumos_df is None or "DESCRIPCION_INSUMO" not in raw_insumos_df.columns:
        logger.error(
            "El DataFrame de insumos crudos no está disponible o no tiene la columna "
            "'DESCRIPCION_INSUMO'. Saltando validación de descripciones."
        )
        return apus_detail_data

    lista_descripciones = (
        raw_insumos_df["DESCRIPCION_INSUMO"].dropna().unique().tolist()
    )
...
def validate_and_clean_data(data_store: dict) -> dict:
    ...
    if "presupuesto" not in data_store:
        logger.error(
            "La clave 'presupuesto' no se encontró en data_store. "
            "Saltando validaciones de presupuesto."
        )
    else:
        data_store["presupuesto"] = _validate_extreme_costs(
            data_store["presupuesto"]
        )

    if "apus_detail" not in data_store:
        logger.error(
            "La clave 'apus_detail' no se encontró en data_store. "
            "Saltando validaciones de APU detail."
        )
    else:
        data_store["apus_detail"] = _validate_zero_quantity_with_cost(
            data_store["apus_detail"]
        )

        raw_insumos_df = data_store.get("raw_insumos_df")
        data_store["apus_detail"] = _validate_missing_descriptions(
            data_store["apus_detail"], raw_insumos_df
        )
