...
# ============================================================================
# ════════════════════════════════════════════════════════════════════════════
# FASE 1 — HERRAMIENTAS FÍSICAS Y MATEMÁTICAS (v4.0.0)
# ════════════════════════════════════════════════════════════════════════════
# ============================================================================
r"""
...
# ────────────────────────────────────────────────────────────────────────────
# §1.0 — CONFIGURACIÓN DEL LOGGING (Base compartida)
# ────────────────────────────────────────────────────────────────────────────
def _configure_logger(name: str, log_dir: str = "logs") -> logging.Logger:
    ...
    Invariante: El logger preserva la trazabilidad de todas las operaciones
    matemáticas críticas para auditoría posterior.
    """
...
# ────────────────────────────────────────────────────────────────────────────
# §1.1 — ÁLGEBRA RIEMANNIANA AUXILIAR (base matemática compartida)
# ────────────────────────────────────────────────────────────────────────────
def _metric_norm(v: np.ndarray, G: np.ndarray) -> float:
    ...
    Esta es la norma inducida por el producto interno métrico G en el
    espacio tangente. Generaliza la norma euclidiana (G=I) al espacio
    deformado por el tensor métrico G_μν.
    
    Propiedades verificadas (axiomas de norma):
        (a) ‖v‖_G ≥ 0                        (definida positiva, G ≻ 0)
        (b) ‖v‖_G = 0 ↔ v = 0               (no degenerada)
        (c) ‖λv‖_G = |λ|·‖v‖_G             (homogeneidad absoluta)
        (d) ‖u + v‖_G ≤ ‖u‖_G + ‖v‖_G      (desigualdad triangular)
...
def _metric_dual(v: np.ndarray, G: np.ndarray) -> np.ndarray:
    ...
    Operación musical ♭ (flat) en geometría Riemanniana:
        ♭ : T_xM → T*_xM
        v ↦ v♭ = G · v   ∈ T*_xM   (covector / 1-forma)
    ...
    Importancia física (teoría de campos):
...
class FrictionCalculator:
    ...
    # ─────────────────────────────────────────────────────────────────────
    # §1.1.1 — Función signo suavizada COVARIANTE
    # ─────────────────────────────────────────────────────────────────────
    @staticmethod
    ...
    def smooth_sign_metric(
        ...
        Lanza
        -----
        ValueError : Si G no es cuadrada o dimensionalmente incompatible con v.
        """
        v_arr  = np.asarray(v,  dtype=np.float64)
        G_arr  = np.asarray(G,  dtype=np.float64)
    ...
    # ─────────────────────────────────────────────────────────────────────
    # §1.1.2 — Cálculo de fricción covariante (SUTURA 1 PRINCIPAL)
    # ─────────────────────────────────────────────────────────────────────
    @staticmethod
    def compute_friction(
        velocity        : float,
        driving_force   : float,
        friction_model  : Any,  # FrictionModel enum
        ...
        # §1.1.2.1: Construcción del tensor métrico
        ...
        if friction_model == FrictionModel.COULOMB:
            if is_kinetic:
                ...
                # §1.1.2.4: Régimen estático — fricción limitada por F_drv
        ...
        if friction_model == FrictionModel.STRIBECK:
            ...
            if is_kinetic:
                # §1.1.2.5: Magnitud Stribeck en función de la norma métrica
                # F_c(‖v‖_G) = F_C + (F_S-F_C)·exp(-(‖v‖_G/v_St)²)
                ...
                # §1.1.2.6: Régimen estático Stribeck — límite = F_S
                norm_G_f = _metric_norm(f_vec, G)
                if norm_G_f < 1e-15:
                    return 0.0
                static_covector = (
    ...
    # ─────────────────────────────────────────────────────────────────────
    # §1.1.3 — Cálculo batch (múltiples velocidades simultáneas)
    # ─────────────────────────────────────────────────────────────────────
    @staticmethod
...
class LieGroupPIDController:
    ...
    # ─────────────────────────────────────────────────────────────────────
    # §1.2.1 — Logaritmo de Riemann (error geodésico)
    # ─────────────────────────────────────────────────────────────────────
    def _lie_log(
        self,
        x: np.ndarray,
    ...
    # ─────────────────────────────────────────────────────────────────────
    # §1.2.2 — Transporte paralelo (acumulación integral isométrica)
    # ─────────────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────────────
    # §1.2.3 — Derivada covariante filtrada (término D del PID geométrico)
    # ─────────────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────────────
    # §1.2.4 — Actualización del controlador PID geométrico
    # ─────────────────────────────────────────────────────────────────────
    def update(
        self,
        setpoint         : float,
        ...
        # §1.2.4.1: Vectores en g (álgebra de Lie, dim=state_dim)
        ...
        # §1.2.4.2: Error geodésico ξ = Log_x(x_d) ∈ g
        xi = self._lie_log(x, x_d)  # shape (n,)
        
        # §1.2.4.3: Transporte paralelo y acumulación integral en g
        ...
        # §1.2.4.4: Derivada covariante filtrada D_f
        D_f = self._update_covariant_derivative(vel, dt)  # shape (n,)
        
        # §1.2.4.5: Señal de control en g
        # u = -Kp·ξ - Kd·D_f - Ki·σ
        u_vec = (
            - self.kp * xi
            - self.kd * D_f
        ...
        # §1.2.4.6: Anti-windup geométrico
        ...
        # §1.2.4.7: Retornar componente escalar (sistema 1D del pistón)
...
class AtomicPiston:
    ...
    def __init__(
        ...
        mode                 : Any = None,  # PistonMode enum
        transducer_type      : Any = None,  # TransducerType enum
        friction_model       : Any = None,  # FrictionModel enum
        coulomb_friction     : float = 0.2,
        stribeck_coeffs      : Tuple[float, float, float] = (0.3, 0.1, 0.05),
        ...
        from .constants import PistonMode, TransducerType, FrictionModel
        ...
        self.mode             : PistonMode     = mode if mode else PistonMode.CAPACITOR
        self.transducer_type  : TransducerType = transducer_type if transducer_type else TransducerType.PIEZOELECTRIC
        self.friction_model   : FrictionModel  = friction_model if friction_model else FrictionModel.VISCOUS
    ...
    def _configure_transducer(self) -> None:
        ...
        """
        from .constants import TransducerType
    ...
    def discharge(self, dt: float) -> Optional[Dict[str, Any]]:
        ...
        """
        from .constants import PistonMode
    ...
    def set_mode(self, mode: Any) -> None:  # PistonMode enum
...
# ════════════════════════════════════════════════════════════════════════════
# FIN DE FASE 2 — GEMELO DIGITAL SIMPLÉCTICO (v4.0.0)
# ════════════════════════════════════════════════════════════════════════════


# ============================================================================
# ════════════════════════════════════════════════════════════════════════════
# FASE 3 — MICROSERVICIO FLASK + ServiceContext (v4.0.0)
# ════════════════════════════════════════════════════════════════════════════
# ============================================================================
r"""
...
class ServiceContext:
    ...
    """
    config            : Optional[Any]          = field(default=None)  # PistonConfig
