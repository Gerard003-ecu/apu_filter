...
| **$\|\Gamma(t)\|_{\max} > 0.5 \tau_{\mathrm{margin}}$** | `set_agent.py` / `set_engine.py` | **Desajuste Crítico de Impedancia en Frontera (TDR):** Reflexión sónica en la frontera de de Rham. | **Sobreprecio u ocultamiento de ítems en pliegos SECOP II:** Alteración de precios unitarios que detona Veto Duro en silicio ($<400\text{ ns}$). |
| **$\delta_{\mathrm{Hurwitz}} > \varepsilon_{\mathrm{Wilk}}$** | `quaternionic_state_shifter.py` | **Deriva de Wilkinson:** Mermas de representabilidad por redondeo en la FPU de 64 bits en H ($\|p \cdot q\|_{\mathbb{H}} \neq \|p\| \|q\|$). | **Inestabilidad de-normalizada:** Alerta previa a la inyección de entropía en la caja de la constructora. |
| **$\det(\sigma') \le 0$** | `hydrological_agent.py` | **Plastificación por Licuación (Fango):** Pérdida total de soporte por presiones de poros intersticiales de Biot-Terzaghi. | **Peligro inminente de colapso de cimientos:** Disparo del Crowbar en silicio (<400 ns) para desenergizar bombas y mezcladoras. |
| **$i_{\mathrm{grad}} > i_{\mathrm{crit}}$** | `hydrological_agent.py` | **Erosión Interna por Sifonamiento:** Arrastre destructivo de finos en foso por gradientes hidráulicos acelerados. | **Deformación geomecánica e inestabilidad:** Daño estructural a cimentaciones de obra civil. |
