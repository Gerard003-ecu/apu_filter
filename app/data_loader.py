...
def _analyze_dataframe_quality(df: pd.DataFrame) -> DataQualityMetrics:
    ...
    empty_cols = [
        col for col in df.columns if not str(col).strip() or str(col).lower() == "nan"
    ]
    if empty_cols:
    ...
    if metrics.total_columns > MAX_COLS_WARNING:
        metrics.add_warning(
            f"Muchas columnas: {metrics.total_columns} (umbral: {MAX_COLS_WARNING})"
        )
...
def _clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df_clean.columns.tolist():
        ...
        if duplicated_mask.any():
            ...
            logger.warning(
                f"Se renombraron columnas duplicadas: {sum(duplicated_mask)} duplicados"
            )
...
def _detect_csv_delimiter(
    path: Path, sample_size: int = 5, encoding: str = "utf-8"
) -> Optional[str]:
    """
    ...
    try:
        ...
        if delimiter_counts:
            ...
            if detected[1] > 0:
                logger.debug(
                    f"Delimitador detectado: '{detected[0]}' (ocurrencias: {detected[1]})"
                )
                return detected[0]
    ...
    for enc in encodings_to_try:
        for delim in delimiters_to_try:
            try:
                ...
                df = pd.read_csv(path, sep=delim, encoding=enc, **kwargs)
...
def load_from_xlsx(
    path: PathType, sheet_name: Optional[Union[str, int, List[str]]] = 0, **kwargs
) -> LoadResult:
    """
                        ...
                        try:
                            ...
                            if len(table) > 1:
                                headers = (
                                    table[0]
                                    if table[0]
                                    else [f"Col_{j}" for j in range(len(table[0] or []))]
                                )
...
# ============================================================================
def load_data(path: PathType, format_hint: Optional[str] = None, **kwargs) -> LoadResult:
    """
    ...
    try:
        ...
        logger.info(
            f"Formato detectado: {file_format.value}, Tamaño: {metadata.size_mb:.2f} MB"
        )
...
def validate_file_before_load(path: PathType) -> Tuple[bool, Optional[str]]:
    ...
    try:
        ...
        if metadata.size_mb > MAX_FILE_SIZE_MB:
            return (
                False,
                f"Archivo demasiado grande: {metadata.size_mb:.2f} MB (máx: {MAX_FILE_SIZE_MB} MB)",
            )
