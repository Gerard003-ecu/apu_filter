...
* **El Soberano Cuaterniónico (Gobernanza de Fase de la FPU):** Representado por `quaternionic_state_agent.py`. Opera en $V_{\mathbb{W}}$ (Nivel 0) gobernando el reactor cuaterniónico $q = q_0 e_0 + q_1 i + q_2 j + q_3 k \in \mathbb{H}$ [quaternionic_state_shifter.py] con relaciones de Hamilton $i^2=j^2=k^2=ijk=-e_0=-1$, evaluando la inmersión de Cayley-Dickson $\iota(q) \in M_2(\mathbb{C})$, la traza de von Neumann $\operatorname{Tr}(\rho) \equiv 1.0$ y la proyección en la 2-esfera de Riemann $S^2 \cong \hat{\mathbb{C}}$ ($Z = \frac{q_1 + \sqrt{-1}q_2}{1 - q_3}$) con desviación $\delta_{\mathrm{similarity}} = |Z|(1 - q_3)$ para inmunizar el cálculo contra la deriva de Wilkinson.
