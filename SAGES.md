...
*   **La Membrana de Ingesta:** El Guardián asume que el mundo real es caótico. Cuando el usuario sube un Excel o CSV complejo, el Guardián no lo rechaza inmediatamente ni permite que sature la memoria. Activa una "Membrana Amortiguadora" que absorbe el impacto físico de los datos masivos, filtra el ruido estructural y estabiliza el volumen de inyección en lotes (batches) controlados por un algoritmo PID antes de que toquen las reglas de negocio.
...
• El Microscopio Topológico (Números de Betti):
    ◦ β0​ (Islas de Datos): Detecta fragmentación. Si β0​>1, existen "recursos huérfanos" (compras desconectadas de la obra).
    ◦ β1​ (Socavones Lógicos): Detecta ciclos. Si β1​>0, hay dependencias circulares (El techo depende del muro, el muro del techo). Bloquea la matemática contable.
...
• Interoperabilidad Semántica (GraphRAG): Utiliza vectores semánticos (Embeddings) para saber que "Concreto 3000 PSI" y "Hormigón Estructural" son el mismo nodo, unificando el lenguaje entre ingenieros y contadores.
...
Todo el ecosistema opera bajo la orquestación del director del pipeline, ejecutando un bucle de control cibernético continuo (OODA) validado por un "Pasabordo Ciber-físico" (TelemetryContext):
1. Observe (Observar): El Guardián ingiere los archivos (Excel/CSV) y mide la "presión" de entrada, estabilizando el flujo mediante control PID.
