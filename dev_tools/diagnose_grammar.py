...
def diagnose_grammar_mismatches(
    csv_file: str, grammar: str, output_file: str = "grammar_diagnosis.txt"
):
    """
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        empty_field_count = sum(
            1 for fl in failed_lines if any(not f.strip() for f in fl["fields"])
        )
        ...
        for fl in failed_lines[:20]:  # Primeras 20
            ...
            empty_positions = [i for i, f in enumerate(fl["fields"]) if not f.strip()]
            if empty_positions:
