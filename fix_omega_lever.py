...
new_improbability = """    @staticmethod
    def _compute_improbability_lever(
        ...
        try:
            ...
            # Extracción segura, forzamos clamp
            penalty = result.penalty if result.success and result.penalty is not None and math.isfinite(result.penalty) else 1.0

            base_lever = (combinatorial_scale * friction_scale * anomaly_pressure) / _IMPROBABILITY_SCALE_FACTOR
            lever = base_lever * penalty
