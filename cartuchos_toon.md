...
Para garantizar que la salida respete el `SiloAContract` original y evitar inyecciones corruptas, el sistema impone una **Condición de Continuidad de Lipschitz** sobre el generador del LLM en tiempo de inferencia. Esto se logra desplegando un **Decodificador de Gramática Restringida** (Constrained Grammar Decoder).
El decodificador fuerza probabilísticamente que la emisión de cualquier token fuera de la variedad tabular TOON válida sea estrictamente nula ($P(x_{invalido}) = 0$). Este anclaje blinda el retracto topológico y garantiza que el dominio del Operador de Expansión ($F^{-1}$) esté siempre matemáticamente bien definido, sin importar el caos atencional del modelo subyacente.
